...
$pickupDateQuery = "SELECT DISTINCT scheduleDay FROM schedule ORDER BY scheduleDay ASC";
...
<body>
                            ...
                            <form id="pickupForm" method="POST" action="php/functions.php?op=confirmBtn">
                                <div class="form-group">
                                    ...
                                    <select id="form_date" name="date" class="form-control" required>
                                        ...
                                        <?php while ($row = mysqli_fetch_assoc($pickupDateResult)): ?>
                                            <?php 
                                                // Convert the date from the database format to display with day name
                                                $formattedDate = date("l, F j, Y", strtotime($row['scheduleDay']));
                                            ?>
                                            <option value="<?= $row['scheduleDay'] ?>">
                                                <?= $formattedDate ?>
                                            </option>
                                        <?php endwhile; ?>
                                ...
                                <div class="col-md-12 my-3">
                                    <div class="form-group">
                                        ...
                                        <select class="form-select form-control" id="wasteType" name="wasteType"
                                            required>
    ...
    const time = document.getElementById("form_time").value;
    const wasteType = document.getElementById("wasteType").value;
    const userID = document.querySelector('input[name="userID"]').value; // Get userID from the hidden input field
